...
Joint::Joint(const unsigned int& id, const vec3& offset)
{
	m_id = id;
	m_offset = offset;
}
...
void AgentDisplay::CreateSkeleton(vector<Joint*>& pJointList)
{
	// Assume pJointList is empty
	Joint* pJoint;
	// Hips
	pJoint = new Joint(eHips, vec3(-29.0441f, 98.1845f, -101.233f));
	pJoint->m_pParent = NULL;
	pJointList.push_back(pJoint);
	// RightUpLeg
	pJoint = new Joint(eRightUpLeg, vec3(-8.9458f, -8.66707f, 0.101652f));
	Joint::AttachJoints(pJointList[eHips], pJoint);
	pJointList.push_back(pJoint);
	// RightLeg
	pJoint = new Joint(eRightLeg, vec3(0.0f, -44.4227f, 1.3727f));
	Joint::AttachJoints(pJointList[eRightUpLeg], pJoint);
	pJointList.push_back(pJoint);
	// RightFoot
	pJoint = new Joint(eRightFoot, vec3(0.253549f, -42.0924f, -1.46059f));
	Joint::AttachJoints(pJointList[eRightLeg], pJoint);
	pJointList.push_back(pJoint);
	// RightToes
	pJoint = new Joint(eRightToes, vec3(0.04f, -4.41509f, 8.4991f));
	Joint::AttachJoints(pJointList[eRightFoot], pJoint);
	pJointList.push_back(pJoint);
	// Spine
	pJoint = new Joint(eSpine, vec3(0.0f, 11.8905f, 0.0f));
	Joint::AttachJoints(pJointList[eHips], pJoint);
	pJointList.push_back(pJoint);
	// Chest
	pJoint = new Joint(eChest, vec3(0.0f, 25.6574f, 0.127663f));
	Joint::AttachJoints(pJointList[eSpine], pJoint);
	pJointList.push_back(pJoint);
	// Neck
	pJoint = new Joint(eNeck, vec3(0.0f, 8.73416f, 0.011341f));
	Joint::AttachJoints(pJointList[eChest], pJoint);
	pJointList.push_back(pJoint);
	// Head
	pJoint = new Joint(eHead, vec3(-0.29f, 12.7742f, 2.51395f));
	Joint::AttachJoints(pJointList[eNeck], pJoint);
	pJointList.push_back(pJoint);
	// LeftShoulder
	pJoint = new Joint(eLeftShoulder, vec3(8.99f, -0.362252f, -0.22f));
	Joint::AttachJoints(pJointList[eChest], pJoint);
	pJointList.push_back(pJoint);
	// LeftArm
	pJoint = new Joint(eLeftArm, vec3(10.2827f, -0.00338f, 0.322123f));
	Joint::AttachJoints(pJointList[eLeftShoulder], pJoint);
	pJointList.push_back(pJoint);
	// LeftForeArm
	pJoint = new Joint(eLeftForeArm, vec3(32.7669f, 0.099144f, -1.42434f));
	Joint::AttachJoints(pJointList[eLeftArm], pJoint);
	pJointList.push_back(pJoint);
	// LeftHand
	pJoint = new Joint(eLeftHand, vec3(27.5643f, 0.0853619f, 1.02217f));
	Joint::AttachJoints(pJointList[eLeftForeArm], pJoint);
	pJointList.push_back(pJoint);
	// RightShoulder
	pJoint = new Joint(eRightShoulder, vec3(-8.99f, -0.362252f, -0.22f));
	Joint::AttachJoints(pJointList[eChest], pJoint);
	pJointList.push_back(pJoint);
	// RightArm
	pJoint = new Joint(eRightArm, vec3(-10.2827f, -0.00338f, 0.322123f));
	Joint::AttachJoints(pJointList[eRightShoulder], pJoint);
	pJointList.push_back(pJoint);
	// RightForeArm
	pJoint = new Joint(eRightForeArm, vec3(-32.8971f, 0.099144f, -1.42434f));
	Joint::AttachJoints(pJointList[eRightArm], pJoint);
	pJointList.push_back(pJoint);
	// RightHand
	pJoint = new Joint(eRightHand, vec3(-27.4921f, 0.0853621f, 1.02217f));
	Joint::AttachJoints(pJointList[eRightForeArm], pJoint);
	pJointList.push_back(pJoint);
	// LeftUpLeg
	pJoint = new Joint(eLeftUpLeg, vec3(8.9458f, -8.66707f, 0.101652f));
	Joint::AttachJoints(pJointList[eHips], pJoint);
	pJointList.push_back(pJoint);
	// LeftLeg
	pJoint = new Joint(eLeftLeg, vec3(0.0f, -44.4227f, 1.3727f));
	Joint::AttachJoints(pJointList[eLeftUpLeg], pJoint);
	pJointList.push_back(pJoint);
	// LeftFoot
	pJoint = new Joint(eLeftFoot, vec3(-0.253549f, -42.0924f, -1.46059f));
	Joint::AttachJoints(pJointList[eLeftLeg], pJoint);
	pJointList.push_back(pJoint);
	// LeftToes
	pJoint = new Joint(eLeftToes, vec3(-0.04f, -4.41509f, 8.4991f));
	Joint::AttachJoints(pJointList[eLeftFoot], pJoint);
	pJointList.push_back(pJoint);
}

void AgentDisplay::DrawBodyParts(Joint* pJoint, float* pJointAngles)
{

	glPushMatrix();
	// Draw limb
	vec3 v = pJoint->m_offset;
	float l = v.Length();
	v /= l;
	...
	glPushMatrix();
	glTranslatef(pJoint->m_offset[VX]/2.0f, pJoint->m_offset[VY]/2.0f, pJoint->m_offset[VZ]/2.0f);
	...
	glPopMatrix();
	// Draw joint
	glTranslatef(pJoint->m_offset[VX], pJoint->m_offset[VY], pJoint->m_offset[VZ]);
	glRotatef(pJointAngles[pJoint->m_id * 3], 0.0f, 0.0f, 1.0f);
	glRotatef(pJointAngles[pJoint->m_id * 3 + 1], 1.0f, 0.0f, 0.0f);
	glRotatef(pJointAngles[pJoint->m_id * 3 + 2], 0.0f, 1.0f, 0.0f);
	glutSolidSphere(6.0, 12, 12);
	if (pJoint->m_pChildren.size() > 0)
	{
		// Iterator all children joints
		for (vector<Joint*>::const_iterator iter = pJoint->m_pChildren.begin(); iter != pJoint->m_pChildren.end(); iter++)
			DrawBodyParts(*iter, pJointAngles);
	}	
	glPopMatrix();
}
...
void AgentDisplay::DrawSkeleton(Joint* pRootJoint, float* pJointAngles)
{
	glPushMatrix();
	// Draw root joint
	glRotatef(pJointAngles[pRootJoint->m_id * 3], 0.0f, 0.0f, 1.0f);
	glRotatef(pJointAngles[pRootJoint->m_id * 3 + 1], 1.0f, 0.0f, 0.0f);
	glRotatef(pJointAngles[pRootJoint->m_id * 3 + 2], 0.0f, 1.0f, 0.0f);
	// Iterator all children joints
	for (vector<Joint*>::const_iterator iter = pRootJoint->m_pChildren.begin(); iter != pRootJoint->m_pChildren.end(); iter++)
		DrawBodyParts(*iter, pJointAngles);
	glPopMatrix();
}
...
{
	vector<Joint*> m_jointList;
	CreateSkeleton(m_jointList);
	float jointAngles[] = {-0.78f, -5.06f, -17.38f, -10.38f, -27.08f, -7.74f, -0.12f, 57.31f, 7.02f, -1.00f, -6.80f, -0.16f, 0.05f, 0.00f, 0.66f, -1.20f, 1.84f, -2.73f, 0.54f, 3.65f, -5.50f, 5.27f, -3.51f, 12.52f, -1.91f, 15.91f, 13.29f, -14.30f, 0.73f, -7.89f, -34.63f, -20.13f, 8.41f, 9.55f, 35.32f, -38.09f, 1.43f, 9.83f, -6.44f, 27.71f, 2.21f, 15.79f, 33.80f, -15.56f, -16.28f, -9.81f, 47.20f, 25.31f, -5.16f, 9.37f, 9.42f, 1.29f, -12.12f, 9.92f, 0.89f, 39.35f, 11.00f, -4.38f, -21.65f, -0.43f, 0.18f, -0.19f, 0.00,
	...
	for (unsigned int i = 0; i < totalFrames; i++)
	{
		...
		DrawSkeleton(m_jointList[0], jointAngles + i * 63);
		glEndList();
	}
	
	for (vector<Joint*>::const_iterator iter = m_jointList.begin(); iter != m_jointList.end(); iter++)
		delete *iter;
	m_jointList.clear();
}
